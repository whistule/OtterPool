...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!signup_id) return err("signup_id is required", 400);
    ...
    if (!signup) return err("Sign-up not found", 404);
    ...
    const event = signup.event as
      ...
      | null;
    if (!event) return err("Event not found for sign-up", 404);
    ...
    if (signup.status !== "pending_review") {
      return err(
        `Sign-up is not pending review (status: ${signup.status})`,
        409,
      );
    }
